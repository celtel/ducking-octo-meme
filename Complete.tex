...
\footnotetext{A list $l=\lbrace l_i\rbrace$ is a subset of list $m=\lbrace m_i\rbrace$ if there exists a subsequence $\lbrace m_{k_i}\rbrace$ of elements from $m$: $m_{k_i}=l_i \forall i$, where $\lbrace k_i\rbrace\subseteq\mathbb{N}$. Note that the definition is for finite and infinite sequences.} Line 6 implies that $compose(installed,\pi)$ returned true, so no conflict with already installed policies. The $for$ loop 9-15 marks then the element containing $\pi$ as true. It means that having the knowledge from all consensus objects C[0] to C[k]$\ni\pi$, $\pi$ was composing,\emph{i.e.}, was not in conflict with all the preceding polices. Contradiction.
\end{proof}
\begin{remark}
